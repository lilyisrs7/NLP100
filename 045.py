from collections import defaultdict

class Morph:
    def __init__(self, surface, base, pos, pos1):
        self.surface = surface
        self.base = base
        self.pos = pos
        self.pos1 = pos1

class Chunk:
    def __init__(self, morphs, dst, srcs):
        self.morphs = morphs
        self.dst = dst
        self.srcs = srcs

def read_ai_chunks(filepath):
    ai = []
    with open(filepath) as f:
        sentence = []
        src_lst = defaultdict(list)
        for line in f.readlines():
            if line.startswith('*'):
                info = line.split()
                dst = int(info[2][:-1])
                src = int(info[1])
                src_lst[dst] = src_lst[dst]+[src]
                chunk = Chunk([], dst, src_lst[src])
                sentence.append(chunk)
            else:
                info = line.rstrip().split('\t')
                if info[0] == 'EOS':
                    if sentence != [] and sentence[-1].morphs != []:
                        ai.append(sentence)
                        sentence = []
                        src_lst = defaultdict(list)
                else:
                    surface = info[0]
                    info_ = info[1].split(',')
                    base = info_[-3]
                    pos = info_[0]
                    pos1 = info_[1]
                    morph = Morph(surface, base, pos, pos1)
                    sentence[-1].morphs = sentence[-1].morphs + [morph]
    return ai

def extract_verb_case_pattern(ai):
    for sentence in ai:
        for chunk in sentence:
            for morph in chunk.morphs:
                if morph.pos == '動詞':
                    verb = morph.base
                    cases = set()
                    for src in chunk.srcs:
                        for src_morph in sentence[src].morphs:
                            if src_morph.pos == '助詞':
                                cases.add(src_morph.surface)
                    if len(cases) > 0:
                        print(verb, ' '.join(sorted(list(cases))), sep='\t')
                    break
    return

ai = read_ai_chunks('ai.ja.txt.parsed')
extract_verb_case_pattern(ai)
# check
# python 045.py > 045.txt
# sort 045.txt | uniq -c | sort -n -r | head -10  > 045_info1.txt
# sort 045.txt | uniq -c | grep '\s行う' | sort -n -r > 045_info2.txt
# sort 045.txt | uniq -c | grep '\sなる' | sort -n -r > 045_info3.txt
# sort 045.txt | uniq -c | grep '\s与える' | sort -n -r > 045_info4.txt